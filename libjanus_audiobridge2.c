/*! \file   janus_audiobridge2.c
 * \author Lorenzo Miniero <lorenzo@meetecho.com>
 * \copyright GNU General Public License v3
 * \brief  Janus audiobridge2 plugin
 ...
 * A pre-filled configuration file is provided in \c conf/janus.plugin.audiobridge2.cfg
 ...
 * audiobridge2 plugin. As anticipated, though, there are also several
...
/* Plugin information */
#define JANUS_audiobridge2_VERSION			8
#define JANUS_audiobridge2_VERSION_STRING	"0.0.8"
#define JANUS_audiobridge2_DESCRIPTION		"This is a plugin implementing an audio conference bridge for Janus, mixing Opus streams."
#define JANUS_audiobridge2_NAME				"JANUS audiobridge2 plugin"
#define JANUS_audiobridge2_AUTHOR			"Meetecho s.r.l."
#define JANUS_audiobridge2_PACKAGE			"janus.plugin.audiobridge2"

/* Plugin methods */
janus_plugin *create(void);
int janus_audiobridge2_init(janus_callbacks *callback, const char *config_path);
void janus_audiobridge2_destroy(void);
int janus_audiobridge2_get_api_compatibility(void);
int janus_audiobridge2_get_version(void);
const char *janus_audiobridge2_get_version_string(void);
const char *janus_audiobridge2_get_description(void);
const char *janus_audiobridge2_get_name(void);
const char *janus_audiobridge2_get_author(void);
const char *janus_audiobridge2_get_package(void);
void janus_audiobridge2_create_session(janus_plugin_session *handle, int *error);
struct janus_plugin_result *janus_audiobridge2_handle_message(janus_plugin_session *handle, char *transaction, char *message, char *sdp_type, char *sdp);
void janus_audiobridge2_setup_media(janus_plugin_session *handle);
void janus_audiobridge2_incoming_rtp(janus_plugin_session *handle, int video, char *buf, int len);
void janus_audiobridge2_incoming_rtcp(janus_plugin_session *handle, int video, char *buf, int len);
void janus_audiobridge2_hangup_media(janus_plugin_session *handle);
void janus_audiobridge2_destroy_session(janus_plugin_session *handle, int *error);
char *janus_audiobridge2_query_session(janus_plugin_session *handle);

/* Plugin setup */
static janus_plugin janus_audiobridge2_plugin =
	JANUS_PLUGIN_INIT (
		.init = janus_audiobridge2_init,
		.destroy = janus_audiobridge2_destroy,

		.get_api_compatibility = janus_audiobridge2_get_api_compatibility,
		.get_version = janus_audiobridge2_get_version,
		.get_version_string = janus_audiobridge2_get_version_string,
		.get_description = janus_audiobridge2_get_description,
		.get_name = janus_audiobridge2_get_name,
		.get_author = janus_audiobridge2_get_author,
		.get_package = janus_audiobridge2_get_package,
		
		.create_session = janus_audiobridge2_create_session,
		.handle_message = janus_audiobridge2_handle_message,
		.setup_media = janus_audiobridge2_setup_media,
		.incoming_rtp = janus_audiobridge2_incoming_rtp,
		.incoming_rtcp = janus_audiobridge2_incoming_rtcp,
		.hangup_media = janus_audiobridge2_hangup_media,
		.destroy_session = janus_audiobridge2_destroy_session,
		.query_session = janus_audiobridge2_query_session,
	);

/* Plugin creator */
janus_plugin *create(void) {
	JANUS_LOG(LOG_VERB, "%s created!\n", JANUS_audiobridge2_NAME);
	return &janus_audiobridge2_plugin;
}
...
static void *janus_audiobridge2_handler(void *data);
static void janus_audiobridge2_relay_rtp_packet(gpointer data, gpointer user_data);
static void *janus_audiobridge2_mixer_thread(void *data);
static void *janus_audiobridge2_participant_thread(void *data);

typedef struct janus_audiobridge2_message {
	janus_plugin_session *handle;
	char *transaction;
	json_t *message;
	char *sdp_type;
	char *sdp;
} janus_audiobridge2_message;
static GAsyncQueue *messages = NULL;
static janus_audiobridge2_message exit_message;

static void janus_audiobridge2_message_free(janus_audiobridge2_message *msg) {
...
typedef struct janus_audiobridge2_room {
	...
	gchar *record_path;			/* Path of the recording file */
	...
	TData * data;
} janus_audiobridge2_room;
static GHashTable *rooms;
static janus_mutex rooms_mutex;

typedef struct janus_audiobridge2_session {
	janus_plugin_session *handle;
...
} janus_audiobridge2_session;
static GHashTable *sessions;
static GList *old_sessions;
static janus_mutex sessions_mutex;

typedef struct janus_audiobridge2_rtp_context {
	/* Needed to fix seq and ts in case of publisher switching */
	uint32_t a_last_ssrc, a_last_ts, a_base_ts, a_base_ts_prev;
	uint16_t a_last_seq, a_base_seq, a_base_seq_prev;
} janus_audiobridge2_rtp_context;

typedef struct janus_audiobridge2_participant {
	janus_audiobridge2_session *session;
	janus_audiobridge2_room *room;	/* Room */
	...
	janus_audiobridge2_rtp_context context;	/* Needed in case the participant changes room */
	/* Opus stuff */
...
} janus_audiobridge2_participant;

/* Packets we get from gstreamer and relay */
typedef struct janus_audiobridge2_rtp_relay_packet {
	rtp_header *data;
	gint length;
	uint32_t ssrc;
	uint32_t timestamp;
	uint16_t seq_number;
} janus_audiobridge2_rtp_relay_packet;

/* Helper to sort incoming RTP packets by sequence numbers */
static gint janus_audiobridge2_rtp_sort(gconstpointer a, gconstpointer b) {
	janus_audiobridge2_rtp_relay_packet *pkt1 = (janus_audiobridge2_rtp_relay_packet *)a;
	janus_audiobridge2_rtp_relay_packet *pkt2 = (janus_audiobridge2_rtp_relay_packet *)b;
...
/* Error codes */
#define JANUS_audiobridge2_ERROR_UNKNOWN_ERROR	499
#define JANUS_audiobridge2_ERROR_NO_MESSAGE		480
#define JANUS_audiobridge2_ERROR_INVALID_JSON	481
#define JANUS_audiobridge2_ERROR_INVALID_REQUEST	482
#define JANUS_audiobridge2_ERROR_MISSING_ELEMENT	483
#define JANUS_audiobridge2_ERROR_INVALID_ELEMENT	484
#define JANUS_audiobridge2_ERROR_NO_SUCH_ROOM	485
#define JANUS_audiobridge2_ERROR_ROOM_EXISTS		486
#define JANUS_audiobridge2_ERROR_NOT_JOINED		487
#define JANUS_audiobridge2_ERROR_LIBOPUS_ERROR	488
#define JANUS_audiobridge2_ERROR_UNAUTHORIZED	489
#define JANUS_audiobridge2_ERROR_ID_EXISTS		490
#define JANUS_audiobridge2_ERROR_ALREADY_JOINED	491


/* audiobridge2 watchdog/garbage collector (sort of) */
void *janus_audiobridge2_watchdog(void *data);
void *janus_audiobridge2_watchdog(void *data) {
	JANUS_LOG(LOG_INFO, "audiobridge2 watchdog started\n");
	dostart();
	gint64 now = 0;
	while(g_atomic_int_get(&initialized) && !g_atomic_int_get(&stopping)) {
		...
		if(old_sessions != NULL) {
			...
			JANUS_LOG(LOG_HUGE, "Checking %d old audiobridge2 sessions...\n", g_list_length(old_sessions));
			while(sl) {
				janus_audiobridge2_session *session = (janus_audiobridge2_session *)sl->data;
				if(!session) {
					sl = sl->next;
					continue;
				}
				if(now-session->destroyed >= 5*G_USEC_PER_SEC) {
					...
					JANUS_LOG(LOG_VERB, "Freeing old audiobridge2 session\n");
					GList *rm = sl->next;
	...
	JANUS_LOG(LOG_INFO, "audiobridge2 watchdog stopped\n");
	return NULL;
}
...
int janus_audiobridge2_init(janus_callbacks *callback, const char *config_path) {
	...
	/* Read configuration */
	char filename[255];
	g_snprintf(filename, 255, "%s/%s.cfg", config_path, JANUS_audiobridge2_PACKAGE);
	...
	messages = g_async_queue_new_full((GDestroyNotify) janus_audiobridge2_message_free);
	...
	if(config != NULL) {
		...
		while(cl != NULL) {
			...
			janus_config_item *recpath = janus_config_get_item(cat, "record_path");
			...
			janus_audiobridge2_room *audiobridge2 = g_malloc0(sizeof(janus_audiobridge2_room));
			if(audiobridge2 == NULL) {
				JANUS_LOG(LOG_FATAL, "Memory error!\n");
				cl = cl->next;
				continue;
			}
			audiobridge2->room_id = atol(cat->name);
			...
			audiobridge2->room_name = description;
			audiobridge2->is_private = priv && priv->value && janus_is_true(priv->value);
			audiobridge2->sampling_rate = atol(sampling->value);
			switch(audiobridge2->sampling_rate) {
				case 8000:
				case 12000:
				case 16000:
				case 24000:
				case 48000:
					JANUS_LOG(LOG_VERB, "Sampling rate for mixing: %"SCNu32"\n", audiobridge2->sampling_rate);
					break;
				default:
			...
			if(secret != NULL && secret->value != NULL) {
				audiobridge2->room_secret = g_strdup(secret->value);
			}
			if(pin != NULL && pin->value != NULL) {
				audiobridge2->room_pin = g_strdup(pin->value);
			}
			audiobridge2->record = FALSE;
			if(record && record->value && janus_is_true(record->value))
				audiobridge2->record = TRUE;
			if(recfile && recfile->value)
				audiobridge2->record_file = g_strdup(recfile->value);
			if(recpath && recpath->value)
				audiobridge2->record_path = g_strdup(recpath->value);
			audiobridge2->recording = NULL;
			audiobridge2->destroy = 0;
			audiobridge2->participants = g_hash_table_new(NULL, NULL);
			audiobridge2->destroyed = 0;
			janus_mutex_init(&audiobridge2->mutex);
			JANUS_LOG(LOG_VERB, "Created audiobridge2: %"SCNu64" (%s, %s, secret: %s, pin: %s)\n",
				audiobridge2->room_id, audiobridge2->room_name,
				audiobridge2->is_private ? "private" : "public",
				audiobridge2->room_secret ? audiobridge2->room_secret : "no secret",
				audiobridge2->room_pin ? audiobridge2->room_pin : "no pin");
			/* We need a thread for the mix */
			GError *error = NULL;
			audiobridge2->thread = g_thread_try_new("audiobridge2 mixer thread", &janus_audiobridge2_mixer_thread, audiobridge2, &error);
			if(error != NULL) {
				/* FIXME We should clear some resources... */
				JANUS_LOG(LOG_ERR, "Got error %d (%s) trying to launch the mixer thread...\n", error->code, error->message ? error->message : "??");
			} else {
				janus_mutex_lock(&rooms_mutex);
				g_hash_table_insert(rooms, GUINT_TO_POINTER(audiobridge2->room_id), audiobridge2);
	...
	while (g_hash_table_iter_next(&iter, NULL, &value)) {
		janus_audiobridge2_room *ar = value;
	...
	watchdog = g_thread_try_new("abridge watchdog", &janus_audiobridge2_watchdog, NULL, &error);
	if(error != NULL) {
		g_atomic_int_set(&initialized, 0);
		JANUS_LOG(LOG_ERR, "Got error %d (%s) trying to launch the audiobridge2 watchdog thread...\n", error->code, error->message ? error->message : "??");
		janus_config_destroy(config);
		return -1;
	}
	/* Launch the thread that will handle incoming messages */
	handler_thread = g_thread_try_new("janus audiobridge2 handler", janus_audiobridge2_handler, NULL, &error);
	if(error != NULL) {
		g_atomic_int_set(&initialized, 0);
		JANUS_LOG(LOG_ERR, "Got error %d (%s) trying to launch the audiobridge2 handler thread...\n", error->code, error->message ? error->message : "??");
		janus_config_destroy(config);
		return -1;
	}
...
int janus_audiobridge2_get_version(void) {
	return JANUS_audiobridge2_VERSION;
}

const char *janus_audiobridge2_get_version_string(void) {
	return JANUS_audiobridge2_VERSION_STRING;
}

const char *janus_audiobridge2_get_description(void) {
	return JANUS_audiobridge2_DESCRIPTION;
}

const char *janus_audiobridge2_get_name(void) {
	return JANUS_audiobridge2_NAME;
}

const char *janus_audiobridge2_get_author(void) {
	return JANUS_audiobridge2_AUTHOR;
}

const char *janus_audiobridge2_get_package(void) {
	return JANUS_audiobridge2_PACKAGE;
}
...
void janus_audiobridge2_create_session(janus_plugin_session *handle, int *error) {
	...
	janus_audiobridge2_session *session = (janus_audiobridge2_session *)g_malloc0(sizeof(janus_audiobridge2_session));
	if(session == NULL) {
		JANUS_LOG(LOG_FATAL, "Memory error!\n");
		*error = -2;
		return;
...
void janus_audiobridge2_destroy_session(janus_plugin_session *handle, int *error) {
	...
	janus_audiobridge2_session *session = (janus_audiobridge2_session *)handle->plugin_handle; 
	if(!session) {
		JANUS_LOG(LOG_ERR, "No audiobridge2 session associated with this handle...\n");
		*error = -2;
		return;
	}
	JANUS_LOG(LOG_VERB, "Removing audiobridge2 session...\n");
	janus_mutex_lock(&sessions_mutex);
	if(!session->destroyed) {
		g_hash_table_remove(sessions, handle);
		janus_audiobridge2_hangup_media(handle);
...
char *janus_audiobridge2_query_session(janus_plugin_session *handle) {
	...
	janus_audiobridge2_participant *participant = (janus_audiobridge2_participant *)session->participant;
	json_object_set_new(info, "state", json_string(participant && participant->room ? "inroom" : "idle"));
	if(participant) {
		janus_audiobridge2_room *room = participant->room; 
...
struct janus_plugin_result *janus_audiobridge2_handle_message(janus_plugin_session *handle, char *transaction, char *message, char *sdp_type, char *sdp) {
	...
	if(message == NULL) {
		...
		error_code = JANUS_audiobridge2_ERROR_NO_MESSAGE;
	...
	if(!session) {
		JANUS_LOG(LOG_ERR, "No session associated with this handle...\n");
		error_code = JANUS_audiobridge2_ERROR_UNKNOWN_ERROR;
	...
	if(!root) {
		...
		error_code = JANUS_audiobridge2_ERROR_INVALID_JSON;
	...
	if(!request) {
		...
		error_code = JANUS_audiobridge2_ERROR_MISSING_ELEMENT;
		g_snprintf(error_cause, 512, "Missing element (request)");
		goto error;
	}
	if(!json_is_string(request)) {
		JANUS_LOG(LOG_ERR, "Invalid element (request should be a string)\n");
		error_code = JANUS_audiobridge2_ERROR_INVALID_ELEMENT;
	...
	if(!strcasecmp(request_text, "create")) {
		/* Create a new audiobridge2 */
		JANUS_LOG(LOG_VERB, "Creating a new audiobridge2\n");
		...
		if(room_id > 0) {
			...
			if(g_hash_table_lookup(rooms, GUINT_TO_POINTER(room_id)) != NULL) {
				...
				error_code = JANUS_audiobridge2_ERROR_ROOM_EXISTS;
		...
		audiobridge2->room_id = room_id;
		char *description = NULL;
		if(desc != NULL && strlen(json_string_value(desc)) > 0) {
			description = g_strdup(json_string_value(desc));
		} else {
			char roomname[255];
			g_snprintf(roomname, 255, "Room %"SCNu64"", audiobridge2->room_id);
		...
		audiobridge2->room_name = description;
		audiobridge2->is_private = is_private ? json_is_true(is_private) : FALSE;
		if(secret)
			audiobridge2->room_secret = g_strdup(json_string_value(secret));
		if(pin)
			audiobridge2->room_pin = g_strdup(json_string_value(pin));
		if(sampling)
			audiobridge2->sampling_rate = json_integer_value(sampling);
		else
			audiobridge2->sampling_rate = 16000;
		switch(audiobridge2->sampling_rate) {
			case 8000:
			case 12000:
			case 16000:
			...
			default:
				...
				JANUS_LOG(LOG_ERR, "Unsupported sampling rate %"SCNu32"...\n", audiobridge2->sampling_rate);
				error_code = JANUS_audiobridge2_ERROR_UNKNOWN_ERROR;
				g_snprintf(error_cause, 512, "We currently only support 16kHz (wideband) as a sampling rate for audio rooms, %"SCNu32" TBD...", audiobridge2->sampling_rate);
				goto error;
		}
		audiobridge2->record = FALSE;
		if(record && json_is_true(record))
			audiobridge2->record = TRUE;
		if(recfile)
			audiobridge2->record_file = g_strdup(json_string_value(recfile));
		audiobridge2->recording = NULL;
		audiobridge2->destroy = 0;
		audiobridge2->participants = g_hash_table_new(NULL, NULL);
		audiobridge2->destroyed = 0;
		...
		if(error != NULL) {
			...
			g_free(audiobridge2->room_name);
			g_free(audiobridge2->room_secret);
			g_free(audiobridge2->record_file);
			g_hash_table_destroy(audiobridge2->participants);
			g_free(audiobridge2);
			goto error;
		} else {
			g_hash_table_insert(rooms, GUINT_TO_POINTER(audiobridge2->room_id), audiobridge2);
		...
		if(save) {
			...
			JANUS_LOG(LOG_VERB, "Saving room %"SCNu64" permanently in config file\n", audiobridge2->room_id);
			janus_mutex_lock(&config_mutex);
			char cat[BUFSIZ], value[BUFSIZ];
			/* The room ID is the category */
			g_snprintf(cat, BUFSIZ, "%"SCNu64, audiobridge2->room_id);
			janus_config_add_category(config, cat);
			/* Now for the values */
			janus_config_add_item(config, cat, "description", audiobridge2->room_name);
			if(audiobridge2->is_private)
				janus_config_add_item(config, cat, "is_private", "yes");
			g_snprintf(value, BUFSIZ, "%"SCNu32, audiobridge2->sampling_rate);
			janus_config_add_item(config, cat, "sampling_rate", value);
			if(audiobridge2->room_secret)
				janus_config_add_item(config, cat, "secret", audiobridge2->room_secret);
			if(audiobridge2->room_pin)
				janus_config_add_item(config, cat, "pin", audiobridge2->room_pin);
			if(audiobridge2->record_file) {
				janus_config_add_item(config, cat, "record", "yes");
				janus_config_add_item(config, cat, "record_file", audiobridge2->record_file);
			}
			/* Save modified configuration */
			janus_config_save(config, config_folder, JANUS_audiobridge2_PACKAGE);
		...
		json_object_set_new(response, "audiobridge2", json_string("created"));
		json_object_set_new(response, "room", json_integer(audiobridge2->room_id));
		goto plugin_response;
	} else if(!strcasecmp(request_text, "destroy")) {
		JANUS_LOG(LOG_VERB, "Attempt to destroy an existing audiobridge2 room\n");
		json_t *room = json_object_get(root, "room");
		if(!room) {
		...
		janus_mutex_lock(&rooms_mutex);
		janus_audiobridge2_room *audiobridge2 = g_hash_table_lookup(rooms, GUINT_TO_POINTER(room_id));
		if(audiobridge2 == NULL) {
			janus_mutex_unlock(&rooms_mutex);
			JANUS_LOG(LOG_ERR, "No such room (%"SCNu64")\n", room_id);
			error_code = JANUS_audiobridge2_ERROR_NO_SUCH_ROOM;
		...
		if(audiobridge2->room_secret) {
			...
			if(!janus_strcmp_const_time(audiobridge2->room_secret, json_string_value(secret))) {
				janus_mutex_unlock(&rooms_mutex);
				JANUS_LOG(LOG_ERR, "Unauthorized (wrong secret)\n");
				error_code = JANUS_audiobridge2_ERROR_UNAUTHORIZED;
				g_snprintf(error_cause, 512, "Unauthorized (wrong secret)");
				goto error;
			}
		}
		/* Remove room */
	if (audiobridge2->data!=NULL) {
		doend(audiobridge2->data);
		audiobridge2->data = NULL;
	}
		...
		response = json_object();
		json_object_set_new(response, "audiobridge2", json_string("destroyed"));
		...
		GHashTableIter iter;
		gpointer value;
		g_hash_table_iter_init(&iter, audiobridge2->participants);
		while (g_hash_table_iter_next(&iter, NULL, &value)) {
			janus_audiobridge2_participant *p = value;
			if(p && p->session) {
				p->room = NULL;
				int ret = gateway->push_event(p->session->handle, &janus_audiobridge2_plugin, NULL, response_text, NULL, NULL);
				...
				while(p->inbuf) {
					...
					janus_audiobridge2_rtp_relay_packet *pkt = (janus_audiobridge2_rtp_relay_packet *)first->data;
		...
		audiobridge2->destroyed = janus_get_monotonic_time();
		g_thread_join(audiobridge2->thread);
		/* Done */
		JANUS_LOG(LOG_VERB, "audiobridge2 room destroyed\n");
		goto plugin_response;
		...
		g_hash_table_iter_init(&iter, rooms);
		while(g_hash_table_iter_next(&iter, NULL, &value)) {
			janus_audiobridge2_room *room = value;
			if(!room)
				continue;
			if(room->is_private) {
				/* Skip private room */
		...
		janus_mutex_unlock(&rooms_mutex);
		response = json_object();
		json_object_set_new(response, "audiobridge2", json_string("success"));
		...
		janus_mutex_lock(&audiobridge2->mutex);
		json_t *list = json_array();
		GHashTableIter iter;
		gpointer value;
		g_hash_table_iter_init(&iter, audiobridge2->participants);
		while (!audiobridge2->destroyed && g_hash_table_iter_next(&iter, NULL, &value)) {
			janus_audiobridge2_participant *p = value;
		...
		janus_mutex_unlock(&audiobridge2->mutex);
		response = json_object();
		json_object_set_new(response, "audiobridge2", json_string("participants"));
		json_object_set_new(response, "room", json_integer(room_id));
		...
		if(participant == NULL || participant->room == NULL) {
			...
			error_code = JANUS_audiobridge2_ERROR_NOT_JOINED;
		...
		goto plugin_response;
			...
			|| !strcasecmp(request_text, "startRecord")|| !strcasecmp(request_text, "stopRecord")) {
		/* These messages are handled asynchronously */
		janus_audiobridge2_message *msg = g_malloc0(sizeof(janus_audiobridge2_message));
		if(msg == NULL) {
			JANUS_LOG(LOG_FATAL, "Memory error!\n");
			error_code = JANUS_audiobridge2_ERROR_UNKNOWN_ERROR;
			g_snprintf(error_cause, 512, "Memory error");
		...
		error_code = JANUS_audiobridge2_ERROR_INVALID_REQUEST;
...
error:
		{
			...
			json_object_set_new(event, "audiobridge2", json_string("event"));
...
void janus_audiobridge2_incoming_rtp(janus_plugin_session *handle, int video, char *buf, int len) {
	...
	if(participant->active && participant->decoder) {
		...
		janus_audiobridge2_rtp_relay_packet *pkt = g_malloc0(sizeof(janus_audiobridge2_rtp_relay_packet));
		if(pkt == NULL) {
			JANUS_LOG(LOG_FATAL, "Memory error!\n");
			return;
		}
		...
		participant->inbuf = g_list_insert_sorted(participant->inbuf, pkt, &janus_audiobridge2_rtp_sort);
...
void janus_audiobridge2_hangup_media(janus_plugin_session *handle) {
	...
	janus_audiobridge2_participant *participant = (janus_audiobridge2_participant *)session->participant;
	janus_audiobridge2_room *audiobridge2 = participant->room;
	if(audiobridge2 != NULL) {
		janus_mutex_lock(&audiobridge2->mutex);
		json_t *event = json_object();
		json_object_set_new(event, "audiobridge2", json_string("event"));
		json_object_set_new(event, "room", json_integer(audiobridge2->room_id));
		...
		json_decref(event);
		g_hash_table_remove(audiobridge2->participants, GUINT_TO_POINTER(participant->user_id));
if (audiobridge2->data!=NULL) {
   if ( g_hash_table_size (audiobridge2->participants)==0) {
      doend(audiobridge2->data);
      audiobridge2->data=NULL;
   }
}
		...
		while (g_hash_table_iter_next(&iter, NULL, &value)) {
			...
			int ret = gateway->push_event(p->session->handle, &janus_audiobridge2_plugin, NULL, leaving_text, NULL, NULL);
			JANUS_LOG(LOG_VERB, "  >> %d (%s)\n", ret, janus_get_api_error(ret));
		}
		g_free(leaving_text);
	}
...
static void *janus_audiobridge2_handler(void *data) {
	JANUS_LOG(LOG_VERB, "Joining audiobridge2 handler thread\n");
	janus_audiobridge2_message *msg = NULL;
	int error_code = 0;
	...
	while(g_atomic_int_get(&initialized) && !g_atomic_int_get(&stopping)) {
		...
		if(msg == &exit_message)
			break;
		if(msg->handle == NULL) {
			janus_audiobridge2_message_free(msg);
			continue;
		}
		janus_audiobridge2_session *session = NULL;
		janus_mutex_lock(&sessions_mutex);
		if(g_hash_table_lookup(sessions, msg->handle) != NULL ) {
			session = (janus_audiobridge2_session *)msg->handle->plugin_handle;
		}
		janus_mutex_unlock(&sessions_mutex);
		if(!session) {
			JANUS_LOG(LOG_ERR, "No session associated with this handle...\n");
		...
		if(!strcasecmp(request_text, "join")) {
			...
			janus_audiobridge2_participant *participant = session->participant;
			if(participant != NULL && participant->room != NULL) {
				JANUS_LOG(LOG_ERR, "Already in a room (use changeroom to join another one)\n");
				error_code = JANUS_audiobridge2_ERROR_ALREADY_JOINED;
			...
			if(audiobridge2->room_pin) {
				...
				if(!janus_strcmp_const_time(audiobridge2->room_pin, json_string_value(pin))) {
					janus_mutex_unlock(&rooms_mutex);
			...
			if(id) {
				...
				if(g_hash_table_lookup(audiobridge2->participants, GUINT_TO_POINTER(user_id)) != NULL) {
					/* User ID already taken */
					JANUS_LOG(LOG_ERR, "User ID %"SCNu64" already exists\n", user_id);
					error_code = JANUS_audiobridge2_ERROR_ID_EXISTS;
			...
			if(participant == NULL) {
				participant = g_malloc0(sizeof(janus_audiobridge2_participant));
				if(participant == NULL) {
					JANUS_LOG(LOG_FATAL, "Memory error!\n");
					error_code = JANUS_audiobridge2_ERROR_UNKNOWN_ERROR;
					g_snprintf(error_cause, 512, "Memory error");
			...
			if(participant->decoder == NULL) {
				/* Opus decoder */
				error = 0;
				participant->decoder = opus_decoder_create(audiobridge2->sampling_rate, 1, &error);
				if(error != OPUS_OK) {
					if(participant->display)
						g_free(participant->display);
					if(participant->encoder)
			...
			if(participant->thread == NULL) {
				GError *error = NULL;
				participant->thread = g_thread_try_new("audiobridge2 participant thread", &janus_audiobridge2_participant_thread, participant, &error);
				if(error != NULL) {
			...
			g_hash_table_insert(audiobridge2->participants, GUINT_TO_POINTER(user_id), participant);
			/* Notify the other participants */
			json_t *newuser = json_object();
			json_object_set_new(newuser, "audiobridge2", json_string("joined"));
			json_object_set_new(newuser, "room", json_integer(audiobridge2->room_id));
			...
			while (g_hash_table_iter_next(&iter, NULL, &value)) {
				...
				JANUS_LOG(LOG_VERB, "Notifying participant %"SCNu64" (%s)\n", p->user_id, p->display ? p->display : "??");
				int ret = gateway->push_event(p->session->handle, &janus_audiobridge2_plugin, NULL, newuser_text, NULL, NULL);
				JANUS_LOG(LOG_VERB, "  >> %d (%s)\n", ret, janus_get_api_error(ret));
			}
			g_free(newuser_text);
			...
			event = json_object();
			json_object_set_new(event, "audiobridge2", json_string("joined"));
			json_object_set_new(event, "room", json_integer(audiobridge2->room_id));
			...
			if(muted) {
				...
				json_object_set_new(pub, "audiobridge2", json_string("event"));
				...
				while (g_hash_table_iter_next(&iter, NULL, &value)) {
					...
					JANUS_LOG(LOG_VERB, "Notifying participant %"SCNu64" (%s)\n", p->user_id, p->display ? p->display : "??");
					int ret = gateway->push_event(p->session->handle, &janus_audiobridge2_plugin, NULL, pub_text, NULL, NULL);
					JANUS_LOG(LOG_VERB, "  >> %d (%s)\n", ret, janus_get_api_error(ret));
				}
				g_free(pub_text);
				janus_mutex_unlock(&audiobridge2->mutex);
        ...
        } else if(!strcasecmp(request_text, "stopRecord")) {
            event = json_object();
            janus_audiobridge2_participant *participant = (janus_audiobridge2_participant *)session->participant;
            janus_audiobridge2_room *audiobridge2 = participant->room;
            ...
            json_object_set_new(event, "room", json_integer(participant->room->room_id));
            json_object_set_new(event, "result", json_string("ok"));
        } else if(!strcasecmp(request_text, "startRecord")) {
            janus_audiobridge2_participant *participant = (janus_audiobridge2_participant *)session->participant;
            ...
            if ( audiobridge2->data == NULL) {
                ...
                int ss = g_hash_table_size (audiobridge2->participants);
                int samples = audiobridge2->sampling_rate/50;
                audiobridge2->data =dofirst(ss,audiobridge2->sampling_rate,samples,audiobridge2->record_path);
                ...
                while (!audiobridge2->destroyed && g_hash_table_iter_next(&iter, NULL, &value)) {
                    janus_audiobridge2_participant *p = value;
                    audiobridge2->data->userids[pos] = p->user_id;
                    JANUS_LOG(LOG_WARN, "id %d %d\n", p->user_id,audiobridge2->data->userids[pos]);
                    pos++;
                }
                janus_mutex_unlock(&audiobridge2->mutex);
//                domiddle(audiobridge2->data);
            }
            event = json_object();
            json_object_set_new(event, "audiobridge2", json_string("event"));
            json_object_set_new(event, "room", json_integer(participant->room->room_id));
			...
			janus_audiobridge2_room *old_audiobridge2 = participant->room;
			/* Leave the old room first... */
			janus_mutex_lock(&old_audiobridge2->mutex);
			g_hash_table_remove(old_audiobridge2->participants, GUINT_TO_POINTER(participant->user_id));
if (old_audiobridge2->data!=NULL) {
   if ( g_hash_table_size (old_audiobridge2->participants)==0) {
      doend(old_audiobridge2->data);
      old_audiobridge2->data=NULL;
   }
}
			janus_mutex_unlock(&old_audiobridge2->mutex);
			if(old_audiobridge2->sampling_rate != audiobridge2->sampling_rate) {
				/* Create a new one that takes into account the sampling rate we want now */
				int error = 0;
				OpusEncoder *new_encoder = opus_encoder_create(audiobridge2->sampling_rate, 1, OPUS_APPLICATION_VOIP, &error);
				if(error != OPUS_OK) {
					if(new_encoder)
					...
					janus_mutex_lock(&old_audiobridge2->mutex);
					g_hash_table_insert(audiobridge2->participants, GUINT_TO_POINTER(participant->user_id), participant);
					janus_mutex_unlock(&old_audiobridge2->mutex);
					goto error;
				}
				if(audiobridge2->sampling_rate == 8000) {
				...
				/* Opus decoder */
				error = 0;
				OpusDecoder *new_decoder = opus_decoder_create(audiobridge2->sampling_rate, 1, &error);
				if(error != OPUS_OK) {
					if(new_encoder)
						opus_encoder_destroy(new_encoder);
					new_encoder = NULL;
			...
			event = json_object();
			json_object_set_new(event, "audiobridge2", json_string("event"));
			json_object_set_new(event, "room", json_integer(old_audiobridge2->room_id));
			json_object_set_new(event, "leaving", json_integer(participant->user_id));
			char *leaving_text = json_dumps(event, JSON_INDENT(3) | JSON_PRESERVE_ORDER);
			GHashTableIter iter;
			gpointer value;
			g_hash_table_iter_init(&iter, old_audiobridge2->participants);
			while (g_hash_table_iter_next(&iter, NULL, &value)) {
				janus_audiobridge2_participant *p = value;
				if(p == participant) {
					continue;	/* Skip the new participant itself */
			...
			event = json_object();
			json_object_set_new(event, "audiobridge2", json_string("roomchanged"));
			json_object_set_new(event, "room", json_integer(audiobridge2->room_id));
			json_object_set_new(event, "id", json_integer(user_id));
			json_object_set_new(event, "participants", list);
			janus_mutex_unlock(&audiobridge2->mutex);
...
if (audiobridge2->data!=NULL) {
   if ( g_hash_table_size (audiobridge2->participants)==0) {
      doend(audiobridge2->data);
		...
		/* Any SDP to handle? */
		if(!msg->sdp) {
			int ret = gateway->push_event(msg->handle, &janus_audiobridge2_plugin, msg->transaction, event_text, NULL, NULL);
			JANUS_LOG(LOG_VERB, "  >> %d (%s)\n", ret, janus_get_api_error(ret));
		} else {
			...
			int res = gateway->push_event(msg->handle, &janus_audiobridge2_plugin, msg->transaction, event_text, type, sdp);
...
error:
	...
	g_free(error_cause);
	JANUS_LOG(LOG_VERB, "Leaving audiobridge2 handler thread\n");
	return NULL;
}
...
static void *janus_audiobridge2_mixer_thread(void *data) {
	...
	janus_audiobridge2_room *audiobridge2 = (janus_audiobridge2_room *)data;
	if(!audiobridge2) {
		JANUS_LOG(LOG_ERR, "Invalid room!\n");
		return NULL;
	}
	JANUS_LOG(LOG_VERB, "Thread is for mixing room %"SCNu64" (%s) at rate %"SCNu32"...\n", audiobridge2->room_id, audiobridge2->room_name, audiobridge2->sampling_rate);

	/* Do we need to record the mix? */
	if(audiobridge2->record) {
		char filename[255];
		if(audiobridge2->record_file)
			g_snprintf(filename, 255, "%s", audiobridge2->record_file);
		else
			g_snprintf(filename, 255, "/tmp/janus-audioroom-%"SCNu64".wav", audiobridge2->room_id);
		audiobridge2->recording = fopen(filename, "wb");
		if(audiobridge2->recording == NULL) {
			...
			wav_header header = {
				...
				1,
				audiobridge2->sampling_rate,
				audiobridge2->sampling_rate * 2,
				2,
				16,
				{'d', 'a', 't', 'a'},
				0
			};
			if(fwrite(&header, 1, sizeof(header), audiobridge2->recording) != sizeof(header)) {
	...
	while(!g_atomic_int_get(&stopping) && audiobridge2->destroyed == 0) {	/* FIXME We need a per-room watchdog as well */
		...
		janus_mutex_lock_nodebug(&audiobridge2->mutex);
		count = g_hash_table_size(audiobridge2->participants);
		janus_mutex_unlock_nodebug(&audiobridge2->mutex);
		if(count == 0) {
		...
		janus_mutex_lock_nodebug(&audiobridge2->mutex);
		GList *participants_list = g_hash_table_get_values(audiobridge2->participants);
		janus_mutex_unlock_nodebug(&audiobridge2->mutex);
		for(i=0; i<samples; i++)
			buffer[i] = 0;
		GList *ps = participants_list;
		while(ps) {
			janus_audiobridge2_participant *p = (janus_audiobridge2_participant *)ps->data;
			janus_mutex_lock(&p->qmutex);
			...
			janus_audiobridge2_rtp_relay_packet *pkt = (janus_audiobridge2_rtp_relay_packet *)(peek ? peek->data : NULL);
			if(pkt != NULL) {
...
if ( audiobridge2->data!=NULL ) {
   int i ;
   for(i = 0 ; i<audiobridge2->data->qtparticipants;i++)  {
      if ( p->user_id == audiobridge2->data->userids[i]) {
         domiddle2(audiobridge2->data,i,curBuffer,samples);
         break;
      }
   }
}
		...
		if(audiobridge2->recording != NULL && g_list_length(participants_list) > 0) {
			for(i=0; i<samples; i++) {
				/* FIXME Smoothen/Normalize instead of truncating? */
				outBuffer[i] = buffer[i];
			}
			fwrite(outBuffer, sizeof(opus_int16), samples, audiobridge2->recording);
		}
		/* Send proper packet to each participant (remove own contribution) */
		ps = participants_list;
		while(ps) {
			janus_audiobridge2_participant *p = (janus_audiobridge2_participant *)ps->data;
			janus_audiobridge2_rtp_relay_packet *pkt = NULL;
			janus_mutex_lock(&p->qmutex);
			if(p->active && !p->muted && !p->prebuffering && p->inbuf) {
				GList *first = g_list_first(p->inbuf);
				pkt = (janus_audiobridge2_rtp_relay_packet *)(first ? first->data : NULL);
			...
			janus_audiobridge2_rtp_relay_packet *mixedpkt = g_malloc0(sizeof(janus_audiobridge2_rtp_relay_packet));
			if(mixedpkt != NULL) {
				...
				if(mixedpkt->data == NULL) {
					...
					mixedpkt->ssrc = audiobridge2->room_id;
	...
	if(audiobridge2->recording)
		fclose(audiobridge2->recording);
	JANUS_LOG(LOG_VERB, "Leaving mixer thread for room %"SCNu64" (%s)...\n", audiobridge2->room_id, audiobridge2->room_name);

	/* Free resources */
	g_free(audiobridge2->room_name);
	g_free(audiobridge2->room_secret);
	g_free(audiobridge2->room_pin);
	g_free(audiobridge2->record_file);
	g_free(audiobridge2->record_path);
	g_hash_table_destroy(audiobridge2->participants);
	g_free(audiobridge2);
...
static void *janus_audiobridge2_participant_thread(void *data) {
	JANUS_LOG(LOG_VERB, "audiobridge2 Participant thread starting...\n");
	janus_audiobridge2_participant *participant = (janus_audiobridge2_participant *)data;
	if(!participant) {
	...
	janus_audiobridge2_session *session = participant->session;

	/* Output buffer */
	janus_audiobridge2_rtp_relay_packet *outpkt = g_malloc0(sizeof(janus_audiobridge2_rtp_relay_packet));
	if(outpkt == NULL) {
		JANUS_LOG(LOG_FATAL, "Memory error!\n");
		g_thread_unref(g_thread_self());
		return NULL;
	...
	janus_audiobridge2_rtp_relay_packet *mixedpkt = NULL;
		...
		if(mixedpkt != NULL && session->destroyed == 0) {
			...
			if(participant->active && participant->encoder) {
				...
				if(outpkt->length < 0) {
					...
					/* Backup the actual timestamp and sequence number set by the audiobridge2, in case a room is changed */
					outpkt->ssrc = mixedpkt->ssrc;
					outpkt->timestamp = mixedpkt->timestamp;
					outpkt->seq_number = mixedpkt->seq_number;
					janus_audiobridge2_relay_rtp_packet(participant->session, outpkt);
				}
			}
			if(mixedpkt) {
				if(mixedpkt->data)
	...
	while(g_async_queue_length(participant->outbuf) > 0) {
		janus_audiobridge2_rtp_relay_packet *pkt = g_async_queue_pop(participant->outbuf);
		if(pkt == NULL)
			continue;
		if(pkt->data)
			g_free(pkt->data);
	...
	JANUS_LOG(LOG_VERB, "audiobridge2 Participant thread leaving...\n");
	return NULL;
}

static void janus_audiobridge2_relay_rtp_packet(gpointer data, gpointer user_data) {
	janus_audiobridge2_rtp_relay_packet *packet = (janus_audiobridge2_rtp_relay_packet *)user_data;
	if(!packet || !packet->data || packet->length < 1) {
		JANUS_LOG(LOG_ERR, "Invalid packet...\n");
		return;
	}
	janus_audiobridge2_session *session = (janus_audiobridge2_session *)data;
